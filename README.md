...
- **⚠️ 路径注意事项**：模型在构建时可能引用了构建机器的**本地绝对路径**（外部纹理 / 3D 模型 / 表格文件）。如果在别的电脑上打开后 3D 显示不全或报错，请按以下步骤修复：
  1. 在 FlexSim 中打开模型，查看外部资源引用：**模型 → 模型属性**（或"工具"菜单）中的资源 / 搜索路径列表
  2. 把引用的外部文件复制到模型文件所在目录，并在 FlexSim 中把引用改为**相对路径**
  3. 重新保存模型 —— 之后 `models/` 整个目录即可随模型一起分发
