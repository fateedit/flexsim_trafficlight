...
> **本仓库只包含搭建文档**（不包含 .fsm 模型文件）。按本文步骤操作，即可在 FlexSim 中完整复现三个路口的红绿灯逻辑。
> 需要 FlexSim（商业软件，可用学生版/试用版），建议对 FlexSim 的标签、触发器、AGV 模块有一定了解。

## 仓库结构

```
flexsim-traffic-light/
├── README.md                      ← 本文件：从零搭建指南（简化版，一套代码通吃三个路口）
└── docs/
