...
一个基于 FlexSim 的**简单的红绿灯基础模型**：用 TaskExecuter + DrawSurrogate 搭建三色灯，按设定时长循环切换红/黄/绿，并通过 **Locker / Group / ControlArea** 控制 AGV 在路口的通行与停车。不绑定任何真实地名，按自己的路网即可复用。

> **本仓库包含：搭建文档 + 模型文件**（`models/红绿灯.fsm`，一个简单的红绿灯基础模型示例）。模型直接打开即可运行；文档讲解**通用逻辑与搭建方法**，可复现到你自己的路网。
...
├── models/
│   ├── 红绿灯.fsm                ← 红绿灯基础模型示例（FlexSim 2026 打开）
...
- **模型**：`models/红绿灯.fsm` —— 一个简单的红绿灯基础模型示例（多灯路口布局、AGV 路网与占用/放行逻辑），打开即可运行。
...
| 组成 | 说明 |
|------|------|
| 灯实体 | 1 个 TaskExecuter + 3 个灯色子实体（`red` / `green` / `yellow`） |
| 状态循环 | 绿灯 → 黄灯 → 红灯 → 绿灯，按设定时长循环（`phase` 标签记录状态 + 定时消息切换） |
| 通行控制 | ControlArea（停车区）+ Locker（锁）+ Group（分组）：**绿灯放行、红灯禁行** |

> 下文以一个路口、两个方向组（NS 南北 / EW 东西）为例；灯的数量、方向组、配时按你的路网自定。
...
同一路口所有灯的**周期相等**（周期 = 绿灯 + 黄闪 + 红灯）。各灯用 `偏移时间` 把绿灯起点错开，保证任意时刻只有一个方向组是绿灯：

```
灯 N 的偏移 = 前面所有灯的（绿灯 + 黄闪）之和
```
