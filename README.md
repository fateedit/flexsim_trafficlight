...
一个基于 FlexSim 的**简单的红绿灯基础模型**：用 TaskExecuter + DrawSurrogate 搭建三色灯，通过标签驱动状态机控制灯的轮转，并结合 **Locker / Group / ControlArea** 对 AGV 路径做占用与放行控制。不绑定任何真实地名，按自己的路网即可复用。
...
- **模型**：`models/红绿灯.fsm` —— 一个简单的红绿灯基础模型示例（11 个灯、AGV 路网与占用/放行逻辑），打开即可运行。
