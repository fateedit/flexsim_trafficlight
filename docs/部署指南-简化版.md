# FlexSim 红绿灯系统 — 部署指南（简化版）

> 适用：3 个路口（1号两相位 / 2号四相位 / 3号三相位）
> 本文是 [部署指南-完整版](../README.md) 的压缩版：三个路口的重复脚本收敛为**一套通用代码 + 三张配置表**。三个路口在代码层面只有"配置"不同，按顺序照做即可；需要细节说明时参见完整版。
...
- [1号路口方案](1号路口-两相位方案.md) — 1 号路口的详细配时、脚本与占用逻辑
- [2号路口方案](2号路口-四相位独立灯方案.md) — 2 号路口的四相位独立灯详细方案
- [路口配时参数](路口配时参数.md) — 三个路口配时值的权威计算与校验
- [踩坑记录](踩坑记录.md) — 部署过程中常见错误的排查与修复方法
- Docker部署 — FlexSim 平台的 Docker 容器化部署环境
